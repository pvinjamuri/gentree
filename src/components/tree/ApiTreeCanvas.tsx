...
import { ZoomIn, ZoomOut, Maximize, UserPlus } from 'lucide-react';
...
function layoutTree(members: FamilyMember[], relationships: Relationship[]) {
  ...
  sortedGens.forEach((gen, genIdx) => {
    ...
    genMembers.forEach((m, i) => {
      positioned.push({ member: m, x: startX + i * (NODE_W + H_GAP), y });
    });
  });
  ...
  relationships.filter((r) => r.type === 'spouse').forEach((r) => {
    ...
    if (p1 && p2) {
      lines.push({ id: `spouse-${r.id}`, x1: p1.x + NODE_W, y1: p1.y + NODE_H / 2, x2: p2.x, y2: p2.y + NODE_H / 2, dashed: true });
    }
  });
...
export function ApiTreeCanvas({ slug }: { slug: string }) {
  const router = useRouter();
  const members = useApiStore((s) => s.members);
  const relationships = useApiStore((s) => s.relationships);
  ...
  }, [contentWidth, contentHeight, bounds.minX, bounds.minY, members.length]);
    ...
    return (
      <div className="w-full h-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          {isEditMode && (
            <>
              <Button className="bg-indigo-600 hover:bg-indigo-700 gap-2" onClick={() => setShowAdd(true)}>
  ...
  return (
    ...
    >
      ...
      <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: '0 0', position: 'absolute', zIndex: 1 }}>
        <svg style={{ position: 'absolute', left: bounds.minX, top: bounds.minY, width: contentWidth, height: contentHeight, pointerEvents: 'none', overflow: 'visible' }}>
          {lines.map((line) => (
            <line key={line.id} x1={line.x1 - bounds.minX} y1={line.y1 - bounds.minY} x2={line.x2 - bounds.minX} y2={line.y2 - bounds.minY} stroke={line.dashed ? '#eab308' : '#6366f1'} strokeWidth={2 / zoom} strokeDasharray={line.dashed ? '6 3' : undefined} />
          ))}
        </svg>
        ...
        {positioned.map(({ member, x, y }) => (
          <MemberCard
            ...
            x={x}
            y={y}
            onClick={() => router.push(`/tree/${slug}/member/${member.id}`)}
            onContextMenu={(e) => {
              e.preventDefault();
              e.stopPropagation();
      ...
      <div className="absolute bottom-[72px] left-3 md:bottom-4 z-10 flex flex-col gap-1.5">
        ...
        {isEditMode && (
          <Button variant="outline" size="icon" className="bg-white shadow-md h-10 w-10" onClick={() => setShowAdd(true)}>
      ...
      {contextMenu && (
        <NodeContextMenu x={contextMenu.x} y={contextMenu.y} member={contextMenu.member} onClose={() => setContextMenu(null)} />
      )}
...
function MemberCard({ member, x, y, onClick, onContextMenu }: { member: FamilyMember; x: number; y: number; onClick: () => void; onContextMenu: (e: React.MouseEvent) => void }) {
  ...
  return (
    <div
      ...
      className={cn('bg-white rounded-xl border-2 shadow-md px-4 py-3 cursor-pointer hover:shadow-lg transition-shadow duration-200', isDeceased ? 'border-gray-400 opacity-80' : borderColor)}
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {age !== null && (
            <p className="text-xs text-gray-400">{isDeceased ? `Lived ${age} years` : `Age ${age}`}</p>
          )}
        </div>
      </div>
      {member.location && <p className="text-xs text-gray-400 mt-1 truncate">📍 {member.location}</p>}
    </div>
  );
}
